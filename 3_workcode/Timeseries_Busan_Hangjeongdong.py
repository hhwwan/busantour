...
# PostgreSQL에서 시군구코드 및 행정동코드 매핑 데이터 가져오기
def get_code_mapping():
    connection = connect_postgresql()
    cursor = connection.cursor()

    query = "SELECT 시군구, 시군구코드, 동, LEFT(행정동코드::text, 8) as 행정동코드 FROM code.\"부산_행정동코드\""
    cursor.execute(query)
    data = cursor.fetchall()

    # 시군구 이름에서 "부산광역시 " 제거
    sigungu_mapping = {row[0].replace("부산광역시 ", ""): row[1] for row in data}
    adong_mapping = {row[2]: row[3] for row in data}

    cursor.close()
    connection.close()

    return sigungu_mapping, adong_mapping
...
# MongoDB에 데이터 저장
def transformation(data, update_type, id_value, gugun, adong):
...
# MongoDB meta에 기록
def record_mongo(unique_id, viewname, objecttype, spatiallevel, kpi, nationality, dimension, gugun_code, adong_code):
    # MongoDB 연결 설정
...
def Insert_to_mongo(collection_name):
    ...
    # '행정동'인 경우만 필터링
    filtered_objects = [obj for obj in objects if obj[8] == '행정동']
    ...
    for object in filtered_objects:
        ...
        for adong, adong_data in adongs_data.items():
            ...
            gugun_code = sigungu_mapping.get(gugun)  # 시군구코드 가져오기
            adong_code = adong_mapping.get(adong)  # 행정동코드 가져오기
            unique_id = generate_unique_id(objectid, spatiallevel, kpi, nationality, dimension, gugun_name=gugun, adong_name=adong)  # Unique ID 생성
            for update_type, data in adong_data.items():
                if data is not None:
                    transformation(data, update_type, unique_id, gugun, adong)
            print(f"Data for {unique_id} successfully loaded into MongoDB.")
            record_mongo(unique_id, view_name, objecttype, spatiallevel, kpi, nationality, dimension, gugun_code, adong_code)

def busan_hangjeongdong(collection_name):
