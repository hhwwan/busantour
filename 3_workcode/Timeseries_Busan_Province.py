...
# MongoDB에 데이터 저장
def transformation(data, update_type, id_value):
...
# MongoDB meta에 기록
def record_mongo(unique_id, viewname, objecttype, spatiallevel, kpi, nationality, dimension):
    # MongoDB 연결 설정
...
def Insert_to_mongo(collection_name):
    ...
    for object in filtered_objects:
        ...
        unique_id = generate_unique_id(objectid, spatiallevel, kpi, nationality, dimension)  # Unique ID 생성
        for update_type in ["daily", "weekly", "monthly"]:
            data = fetch_data(update_type, objectid, view_name)
            
            if data is not None and not data.empty:
                transformation(data, update_type, unique_id)
        ...
        record_mongo(unique_id, view_name, objecttype, spatiallevel, kpi, nationality, dimension)

def busan_province(collection_name):
