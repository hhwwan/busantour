...
# PostgreSQL에서 관광지코드 매핑 데이터 가져오기
...
# MongoDB meta에 기록
def record_mongo(unique_id, viewname, objecttype, spatiallevel, kpi, nationality, dimension, attraction_code):
    # MongoDB 연결 설정
...
def Insert_to_mongo(collection_name):
    ...
    for object in filtered_objects:
        ...
        for attraction, attraction_data in attractions_data.items():
            ...
            attraction_code = code_mapping.get(attraction)
            print(f"Data for {unique_id} successfully loaded into MongoDB.")
            record_mongo(unique_id, view_name, objecttype, spatiallevel, kpi, nationality, dimension, attraction_code)

def busan_attraction(collection_name):
