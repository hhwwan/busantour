...
def transform_task(ti):
    ...
    pickle_folder = "TimeSeries_Hangjeongdong_pickle_files"  # 폴더 이름
...
def load_task(ti, collection_name):
    ...
    pickle_folder = "TimeSeries_Hangjeongdong_pickle_files"
    ...
    try:
        for pickle_filepath in transformed_data_paths:
            ...
            sigungu_mapping, adong_mapping = get_code_mapping("adong")
            gugun_code = sigungu_mapping.get(gugun)  # 시군구코드 가져오기
            adong_code = adong_mapping.get(dong)  # 행정동코드 가져오기

            record_mongo(mongo_db, collection_name, unique_id, view_name, objecttype, spatiallevel, kpi, nationality, dimension, gugun_code, adong_code, attraction_code=None)
