...
def generate_unique_id(objectid, spatiallevel, kpi, nationality, dimension, gugun_name=None, adong_name=None, attraction_name=None):
    ...
    if spatiallevel == "구군" and gugun_name:
        # 구군 코드 매핑 가져오기
        gugun_mapping = get_code_mapping("gugun")
        ...
        # 구군 코드 매핑 가져오기
        attraction_mapping = get_code_mapping("attraction")
...
def get_code_mapping(code_type):
    connection = connect_postgresql()
    cursor = connection.cursor()

    if code_type == "gugun":
        query = "SELECT 시군구, 시군구코드 FROM code.\"부산_시군구코드\""
    elif code_type == "adong":
        query = "SELECT 시군구, 시군구코드, 동, LEFT(행정동코드::text, 8) as 행정동코드 FROM code.\"부산_행정동코드\""
    elif code_type == "attraction":
        query = "SELECT 관광지, 관광지코드 FROM code.\"통신사_관광지코드\""
    ...
    if code_type == "gugun":
        ...
        mapping = {row[0]: row[1] for row in data}
