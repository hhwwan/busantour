...
def extract_task():
    ...
    # '부산시전역'인 경우만 필터링
    filtered_objects = [obj for obj in objects if obj[8] == '부산시전역']
...
def transform_task(ti):
    ...
    pickle_folder = "TimeSeries_Province_pickle_files"  # 폴더 이름
    ...
    for obj in data_info:
        ...
        unique_id = generate_unique_id(objectid, spatiallevel, kpi, nationality, dimension)  # 고유 ID 생성

        # 데이터 받아와서 일별, 주별, 월별에 맞게 변환
        for update_type in ["daily", "weekly", "monthly"]:
            data = fetch_data(update_type, objectid, view_name)  # 데이터 가져오기
...
def load_task(ti, collection_name):
    ...
    pickle_folder = "TimeSeries_Province_pickle_files"
    ...
    try:
        for pickle_filepath in transformed_data_paths:
            ...
            # MongoDB안의 'meta'에 기록
            record_mongo(mongo_db, collection_name, unique_id, view_name, objecttype, spatiallevel, kpi, nationality, dimension, gugun_code=None, adong_code=None, attraction_code=None)
    finally:
        mongo_client.close()
